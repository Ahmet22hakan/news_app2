...
import 'package:news_app/blocs/category_tab8_bloc.dart';
...
class CategoryTab8 extends StatefulWidget {
  ...
  CategoryTab8({Key? key, required this.category}) : super(key: key);
...
class _CategoryTab8State extends State<CategoryTab8> {
  ...
  void initState() {
    super.initState();
    if (this.mounted) {
      context.read<CategoryTab8Bloc>().data.isNotEmpty
          ? print('data already loaded')
          : context.read<CategoryTab8Bloc>().getData(mounted, widget.category);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return EasyRefresh(
      ...
      onRefresh: () {
        context.read<CategoryTab8Bloc>().onRefresh(mounted, widget.category);
      },
      onLoad: () {
        context.read<CategoryTab8Bloc>().getMoreData(mounted, widget.category);
      },
